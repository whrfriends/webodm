...
class PluginBase(ABC):
    ...
    def user_can_access(self, request):
        """
        Hook: restrict which users can see and use this plugin.

        Consulted in three places by the framework:
          - app/templatetags/plugins.py  -> hides sidebar menu entries
          - app/plugins/views.py:app_view_handler  -> 404 on URL entry
          - app/plugins/views.py:api_view_handler  -> 404 on /api/... entry

        Default implementation reads the `PluginAccess` admin table
        (one row per plugin, with access_mode + allowed groups).
        Anonymous users always get blocked regardless. Superusers
        always get in.

        Plugins that need custom logic (e.g. row-level per-user data
        scoping) can override this method. The default is intentionally
        data-driven so admins can flip permissions from /admin/ without
        touching plugin code.

        :param request: Django HttpRequest (may be None in rare contexts —
               defensive code should treat that as 'deny')
        :return: bool
        """
        ...
        # Lazy import: avoids AppRegistryNotReady if this hook runs during
        # app-loading. The PluginAccess table is tiny (one row per plugin)
        # and accessed on every plugin-related request, so caching is not
        # worth the staleness risk — admin UI changes take effect instantly.
        try:
            from app.models import PluginAccess
        except Exception:
            return True  # app not fully loaded (migrations running, etc.)

        try:
            access = PluginAccess.objects.get(plugin_name=self.get_name())
        except PluginAccess.DoesNotExist:
            return True  # no row configured → default = public

        if access.access_mode == PluginAccess.ACCESS_PUBLIC:
            return True
        if access.access_mode == PluginAccess.ACCESS_SUPERUSER:
            return False  # non-superuser already checked above
        # ACCESS_RESTRICTED
        return access.groups.filter(pk__in=user.groups.values_list("pk", flat=True)).exists()
